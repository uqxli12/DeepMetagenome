...
def show_info():
    ...
    print(label+"内存占用："+str(memory)+'KB')



#     file = "your_training_data.csv"  # 替换为实际的训练数据文件路径
...
#         self.model.add##
        ...
        self.model.add(LSTM(64,dropout=0.2, recurrent_dropout=0.2))# recurrent_activation = 'sigmoid',
...
#         X_train, X_test, y_train, y_test = train_test_split(self.X, self.Y, test_size=.2)
        ...
        np.savez_compressed('resultsDMP/training_data.npz', X_train=X_train, y_train=y_train)
#         trainingdata=np.load('resultsDMP/training_data.npz',allow_pickle=True)
#         columns_train=trainingdata['columns']
#         values_train=trainingdata['values']
#         from pandas import DataFrame
# #         pd.DataFrame(values_train,columns=columns_train)
#         traindata=pd.DataFrame(values_train,columns=values_train)
#         traindata.to_csv('resultsDMP/trainingdata.csv',index=None)
        np.savez_compressed('resultsDMP/testing_data.npz', X_test=X_test, y_test=y_test)
#         testdata=np.load('resultsDMP/training_data.npz',allow_pickle=True)
#         columns_test=testgdata['columns']
#         values_test=testdata['values']
#         from pandas import DataFrame
# #         pd.DataFrame(values_teat,columns=columns_train)
#         testdata=pd.DataFrame(values_test,columns=values_test)
#         testdata.to_csv('resultsDMP/testdata.csv',index=None)
        ...
        np.savetxt('resultsDMP/predicted_results_test.csv', y_pred_classes, delimiter=',')#TEST predicted
        np.savetxt('resultsDMP/predicted_results_train.csv', y_pred_classes_train, delimiter=',')#TRAIN predicted

        # Save test results
        test_results = pd.DataFrame({'Actual': y_test.flatten(), 'Predicted': y_pred_classes.flatten()})
        test_results.to_csv('resultsDMP/test_results.csv', index=False)
        #save train results
        train_results = pd.DataFrame({'Actual': y_train.flatten(), 'Predicted': y_pred_classes_train.flatten()})
        train_results.to_csv('resultsDMP/train_results.csv', index=False)        
        # 计算召回率和查准率\F1
...
#         sns_plot.figure.savefig(os.path.join(figure_save_path , 'accuracy.png'))#第一个是指存储路径，第二个是图片名字
    ...
    def plt_ROC_curve(self):
        ...
        plt.show()     
    def plt_AUROC_AUPRC_curve(self):
         # Compute AUROC
        fpr, tpr, thresholds = roc_curve(self.y_test, self.test_pred)
        roc_auc = auc(fpr, tpr)

        # Compute precision-recall curve and area under the curve
        precision, recall, _ = precision_recall_curve(self.y_test, self.test_pred)
        pr_auc = auc(recall, precision)

        # Plot AUROC curve
        plt.subplots(1, 1)
        plt.plot(fpr, tpr, color='darkorange', lw=2, label='AUROC curve (area = %0.2f)' % roc_auc)
        plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title('Area Under the Receiver Operating Characteristic Curve')
        plt.legend(loc='lower right')
        plt.savefig('resultsDMP/AUROC.png')
        plt.show()

        # Plot AUPRC curve
        plt.subplots(1, 1)
        plt.plot(recall, precision, color='darkorange', lw=2, label='AUPRC curve (area = %0.2f)' % pr_auc)
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title('Area Under the Precision-Recall Curve')
        plt.legend(loc='lower left')
        plt.savefig('resultsDMP/AUPRC.png')
        plt.show()
